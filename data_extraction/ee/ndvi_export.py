...
def clustered_sample_ndvi_direct_2(feature_coll, debug=False, mask_type='irr',
    ...
    for year in range(start_yr, end_yr + 1):
        ...
        # suggested change from Kel Markert at Google:
        # Not sure if this'll work, it's just trying to implement his comments.
        # Can I mask a whole collection?
        # bands = coll.toBands().rename(ndvi_scenes)
        if mask_type == 'no_mask':
            bands = coll.toBands().rename(ndvi_scenes)
        elif mask_type == 'irr':
            bands = coll.toBands().rename(ndvi_scenes).mask(irr_mask)
        elif mask_type == 'inv_irr':
            bands = coll.toBands().rename(ndvi_scenes).mask(irr.gt(0))
...
def clustered_sample_ndvi_direct_nc(feature_coll, dest_dir, debug=False, mask_type='irr',
                                    start_yr=2004, end_yr=2023, feature_id='FID', drops=None):
    """ Process GEE SEEBOP ndvi data and ... do something with netcdfs?
