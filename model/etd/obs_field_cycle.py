...
def field_day_loop_nc(config, plots, debug_flag=False, params=None):
    ...
    valid_data = plots.input.sel(date=slice(config.start_dt, config.end_dt))  # we'll keep it for now.
    ...
    for j, step_dt in enumerate(plots.input['date'].sel(date=slice(config.start_dt, config.end_dt)).values):
        ...
        foo_day.sdays += 1
        foo_day.dt_string = np.datetime_as_string(step_dt, unit='D')
