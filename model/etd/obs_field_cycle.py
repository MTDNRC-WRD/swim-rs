...
# Use this one now, much faster. (Edited from field_day_loop_nc_4, makes model faster by using dicts.)
def field_day_loop_nc_4_dict(config, plots, params=None, debug_flag=False, save_out=None) -> np.ndarray | xarray.Dataset:
    ...
    if debug_flag:
        ...
        # creating properly-sized arrays for model results.
        eta = np.full((len(time_range), len(targets)), np.nan)
        swe = np.full((len(time_range), len(targets)), np.nan)
    ...
    for j, step_dt in enumerate(time_range):
        ...
        if foo_day.doy == 1 or foo_day.irr_status is None:
            ...
            # irrigated fraction, or bool?
            # foo_day.irr_status = vals['irr']['data'][:, yr_idx]  # fraction
            # foo_day.irr_status = np.where(vals['irr']['data'][:, yr_idx] > config.irr_threshold, 1, 0)  # bool
            foo_day.irr_status = np.where(vals['irr']['data'][:, yr_idx] > 0.01, 1, 0)  # bool
            # We are assuming that every field we use this for will be irrigated. We always want irrigation turned on.
            # This low threshold is just to make sure that fields where Irrmapper says there is zero irrigated fraction
            # still get a valid NDVI crop curve.

        # Using yearly irr_status as condition for which variable type to store for each field on this day
        # need to use irr_threshold as comparison when using irrigated fraction.
        # I think this is the only place where irr_status is used?
        # cond = foo_day.irr_status > config.irr_threshold
        foo_day.ndvi = np.where(foo_day.irr_status, vals['ndvi_irr']['data'][j], vals['ndvi_inv_irr']['data'][j])
        foo_day.capture = np.where(foo_day.irr_status, vals['ndvi_irr_ct']['data'][j],
                                   vals['ndvi_inv_irr_ct']['data'][j])  # why were these etf, and not ndvi?
        # always want to use ET corrected for agricultural land
        foo_day.refet = vals['{}_mm_corrected'.format(config.refet_type)]['data'][j]
        ...
        foo_day.irr_status = np.ones_like(foo_day.irr_status)  # Turn on irrigation always
        ...
        #     foo_day.hr_precip = hr_ppt  # used for calculating infiltration.
        ...
        if debug_flag:
            ...
            for i, fid in enumerate(targets):
                ...
                if abs(balance) > 0.1 and foo_day.year > config.start_dt.year:  # why was this 2000?
                    # pass
                    raise WaterBalanceError('Check November water balance')

        else:
            if np.isnan(tracker.kc_act.any()):
                raise ValueError('NaN in Kc_act')  # why aren't these triggering?

            if np.isnan(tracker.swe.any()):
                raise ValueError('NaN in SWE')

            eta[j, :] = tracker.etc_act  # etc_act = eta_act? (eta_act is not a thing in tracker.)
            swe[j, :] = tracker.swe
