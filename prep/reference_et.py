# TODO: Replace with chmdata version with ability to specify reference ET type.
...
def sunset_angle(sol_dec, lat):
    ...
    #     return arccos(-tan(sol_dec) * tan(lat))
    # else:
    #     intermediate = np.zeros((len(sol_dec), len(lat)))
    #     for i in range(len(lat)):
    #         intermediate[:, i] = -tan(sol_dec) * tan(lat[i])
    #     return arccos(intermediate)
...
def extraterrestrial_r(tindex, lat, shape):
    ...
    """
    # if len(shape) > 1:
    #     j = broadcast_to(day_of_year(tindex), (shape[1], shape[0]))
    ...
    #
    #     return broadcast_to(
    #         118.08 / 3.141592654 * dr * (omega * xx + yy * sin(omega)), (shape[1], shape[0]))
    # else:

    # For multiple fields, only need one one solar_declination
    j = day_of_year(tindex)
    # j = broadcast_to(day_of_year(tindex), shape)
    # j = [day_of_year(tindex)] * shape[1]
    dr = relative_distance(j)
    sol_dec = solar_declination(j)
    ...
    # yy = np.zeros(shape)
    # for i in range(len(lat)):
    #     omega[:, i] = sunset_angle(sol_dec, lat[i])
    #     xx[:, i] = sin(sol_dec) * sin(lat[i])
    #     yy[:, i] = cos(sol_dec) * cos(lat[i])
    #
    # return broadcast_to(
    ...
    if isinstance(lat, (float, int)):
        ...
        xx = sin(sol_dec) * sin(lat)
        yy = cos(sol_dec) * cos(lat)
        ex_r = 118.08 / 3.141592654 * dr * (omega * xx + yy * sin(omega))

    else:
        ex_r = np.zeros(shape)
        for i in range(len(lat)):
            omega = sunset_angle(sol_dec, lat[i])
            xx = sin(sol_dec) * sin(lat[i])
            yy = cos(sol_dec) * cos(lat[i])
            ex_r[:, i] = 118.08 / 3.141592654 * dr * (omega * xx + yy * sin(omega))
    ...
    if rso is None:
        ...
        if isinstance(elevation, (float, int)):
            ...
            for i in range(len(elevation)):
                rso[:, i] = calc_rso(ra=ra[:, i], elevation=elevation[i], kab=kab)
...
def calc_rad_sol_in(tindex, lat, shape, n, as1=0.25, bs1=0.5, nn=None, tmax=None, tmin=None):
    ...
    if nn is None:
        ...
        if isinstance(lat, float):
            ...
            nn = np.zeros(shape)
            for i in range(len(lat)):
                nn[:, i] = daylight_hours(tindex, lat[i])
...
def calc_rso(ra, elevation, kab=None):
    ...
    if kab is None:
        # rso = np.zeros(shape)
        # for i in range(len(elevation)):
        #     rso[:, i] = (0.75 + (2 * 10 ** -5) * elevation) * ra
...
def pm_fao56_ref(tmean, wind, rs=None, rn=None, g=0, tmax=None, tmin=None,
    ...
    if pressure is None:
        ...
        #     pressure = calc_press(elevation)
        # else:
        #     pressure = [calc_press(e) for e in elevation]
    gamma = calc_psy(pressure)
    dlt = calc_vpc(tmean)
...
#
# def pm_fao56_ref_nc(tmean, wind, rs=None, rn=None, g=0, tmax=None, tmin=None,
#                     rhmax=None, rhmin=None, rh=None, pressure=None, elevation=None,
#                     lat=None, n=None, nn=None, rso=None, a=1.35, b=-0.35,
#                     albedo=0.23, ref='grass'):
#     """ Evaporation calculated according to [allen_1998]_.
